  ...
  const handleCreateContestType = async (e) => {
    ...
    try {
      const data = await apiFetch('/api/contest-types', {
        method: 'POST',
        body: JSON.stringify({ name: newContestTypeName, description: newContestTypeDesc })
      });
      if (data.success) {
  ...
  const handleUpdateContestType = async (e) => {
    ...
    try {
      const data = await apiFetch(`/api/contest-types/${editingContestType._id}`, {
        method: 'PUT',
        body: JSON.stringify({ name: editContestTypeName, description: editContestTypeDesc })
      });
      if (data.success) {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8 text-slate-800 dark:text-slate-200">
      ...
      {activeTab === 'categories_config' && (
        <div className="flex flex-col gap-8 animate-in fade-in duration-200">
          ...
          <div className="w-full">
            <div className="glass-panel contest-type-form-container border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-4">
                ...
                <div>
                  {editingContestType ? (
                    <form onSubmit={handleUpdateContestType} className="flex flex-col gap-4 text-left">
                      <div className="flex flex-col gap-1">
                ...
                {/* List Column */}
                <div className="flex flex-col gap-2 text-left border-t md:border-t-0 md:border-l border-slate-100 dark:border-slate-800 pt-6 md:pt-0 md:pl-8">
          ...
          {/* 2-Column Section: Create Category & Categories Ledger */}
