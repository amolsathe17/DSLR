...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (eventData.success && eventData.events.length > 0) {
        activeEvent = eventData.events.find(e => e.status === 'Active') || eventData.events[0];
      ...
      if (activeEvent) {
        const subData = await apiFetch(`/api/submissions/my-submission/${activeEvent._id}`);
        if (subData.success) {
  ...
  const handleDeletePhoto = async (photoId) => {
    if (!confirm('Are you sure you want to remove this photograph from your entry?')) return;
    setError('');

    try {
      const data = await apiFetch(`/api/submissions/photo/${event._id}/${photoId}`, {
        method: 'DELETE'
      });
      if (data.success) {
        setSubmission(data.submission);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (data.success) {
        setSubmission(prev => ({ ...prev, paymentId: data.payment._id }));
        setPaymentSuccess(data.payment);
        setShowQRInvoice(data.payment);
        confetti({ particleCount: 150, spread: 80, duration: 3000 });
        
        // Refresh submission state
        const subData = await apiFetch(`/api/submissions/my-submission/${event._id}`);
        if (subData.success) {
          setSubmission(subData.submission);
        }
      }
  ...
  const handleFinalSubmit = async () => {
    if (!confirm('FINAL SUBMISSION: This will lock all your photos and descriptions for grading. You cannot make changes afterwards. Proceed?')) return;
    setLoading(true);
    setError('');
        ...
        <div className="max-w-4xl mx-auto flex flex-col gap-8">
          ...
          <div className="text-center flex flex-col gap-2">
            ...
            <p className="text-sm text-slate-500 max-w-xl mx-auto">
              Ready to submit your DSLR frames? Choose your package, confirm you follow our DSLR-only camera terms, and initiate your entry folder.
            </p>
          </div>

          <form onSubmit={handleStartSubmission} className="flex flex-col gap-8">
              ...
              {event.packages.map(pkg => (
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">
                    Max Uploads: {pkg.maxPhotos} Photograph{pkg.maxPhotos > 1 ? 's' : ''}
                  </span>
                </label>
              ))}
            </div>
            ...
            <div className="glass-panel border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 sm:p-6 flex flex-col gap-4">
              <div className="flex items-center gap-2 pb-3 border-b border-slate-100 dark:border-slate-800">
                ...
                <h3 className="font-display font-bold text-slate-900 dark:text-white">DSLR Eligibility Declaration</h3>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="glass-panel border border-slate-200 dark:border-slate-800 rounded-2xl p-6 flex flex-col gap-4 shadow-sm">
              <h3 className="font-display font-bold text-slate-900 dark:text-white text-base">Submission Directory</h3>
              
              <div className="flex flex-col gap-3">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">Contest:</span>
                  <span className="font-semibold text-slate-700 dark:text-slate-200 truncate max-w-[180px]">{event.title}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">Package:</span>
                  <span className="font-bold text-indigo-600 dark:text-indigo-400">{selectedPackage?.name}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-semibold text-slate-700 dark:text-slate-200">
                    {submission.photographs.length} of {selectedPackage?.maxPhotos} photos uploaded
                  </span>
                </div>
              </div>
              ...
              <div className="border-t border-slate-100 dark:border-slate-800 pt-4 flex flex-col gap-3">
                ...
                {isPaid && (
                  <button
                    onClick={() => setShowQRInvoice(paymentSuccess || { _id: submission.paymentId })}
                ...
                {!isFinalized && isPaid && submission.photographs.length > 0 && (
                  <button
            ...
            {event.winnersPublished && isFinalized && (
              <div className="glass-panel border border-amber-500/30 bg-amber-500/5 rounded-2xl p-6 flex flex-col gap-4 shadow-sm">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                  The panel has published final scores. You can view your grades, comments, and claim your digital credential.
                </p>
                <button
          ...
          <div className="lg:col-span-8 flex flex-col gap-8">
            ...
            {!isFinalized && submission.photographs.length < selectedPackage?.maxPhotos && (
              <div className="glass-panel border border-slate-200 dark:border-slate-800 rounded-3xl p-6 flex flex-col gap-6 shadow-sm">
                <div className="pb-4 border-b border-slate-100 dark:border-slate-800">
                  <h3 className="font-display font-bold text-slate-900 dark:text-white text-base">Upload Photography Entry</h3>
                  <p className="text-xs text-slate-500">Provide details and select files. We will parse EXIF metadata to auto-fill camera specifications.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-slate-500">Photograph Title</label>
                    <input
                      type="text"
                      value={title}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-slate-500">Camera Brand</label>
                    <input
                      type="text"
                      value={cameraBrand}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-slate-500">Camera Model</label>
                    <input
                      type="text"
                      value={cameraModel}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-slate-500">Lens Model</label>
                    <input
                      type="text"
                      value={lensUsed}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className="text-xs font-semibold text-slate-500">Date Captured</label>
                    <input
                      type="date"
                      value={dateCaptured}
                  ...
                  <div className="flex flex-col gap-1.5 md:col-span-2">
                    <label className="text-xs font-semibold text-slate-500">Capture Location</label>
                    <input
                      type="text"
                      value={location}
                  ...
                  <div className="flex flex-col gap-1.5 md:col-span-2">
                    <label className="text-xs font-semibold text-slate-500">Optional Description</label>
                    <textarea
                      value={description}
                ...
                <div className="border-t border-slate-100 dark:border-slate-800 pt-4 flex flex-col gap-2">
                  <label className="text-xs font-bold text-slate-800 dark:text-slate-200">Select Files</label>
                  <DragDropUpload
            ...
            <div className="flex flex-col gap-4">
              ...
              {submission.photographs.length === 0 ? (
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-12 text-center text-slate-400 flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-sm font-medium">No photos uploaded in this entry folder yet.</p>
                  <p className="text-xs max-w-xs text-slate-500">Use the entry form above to upload photographs corresponding to your selected package tier.</p>
                </div>
              ) : (
                    ...
                    >
                      ...
                      <div className="p-4 flex flex-col gap-3 flex-grow justify-between">
                        <div>
                          ...
                          <div className="grid grid-cols-2 gap-x-3 gap-y-1.5 mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 text-[10px] text-slate-500">
                            <div>
                              ...
                              <p className="font-semibold text-slate-700 dark:text-slate-300 truncate">{photo.cameraBrand} {photo.cameraModel}</p>
                            </div>
                            <div>
                              <span className="text-slate-400">Lens:</span>
                              <p className="font-semibold text-slate-700 dark:text-slate-300 truncate">{photo.lensUsed || 'N/A'}</p>
                            </div>
                            <div>
                              <span className="text-slate-400">Location:</span>
                              <p className="font-semibold text-slate-700 dark:text-slate-300 truncate">{photo.location || 'N/A'}</p>
                            </div>
                            <div>
                        ...
                        <div className="border-t border-slate-100 dark:border-slate-800 pt-3 mt-3 flex items-center justify-between gap-4">
                          <div>
                            <span className="text-[9px] text-slate-400 uppercase tracking-wider block font-bold">Review Status</span>
                            ...
                            {photo.status === 'Rejected' && (
                              <div className="mt-0.5">
                                ...
                                <p className="text-[9px] text-red-500 mt-1 leading-snug">
                                  Reason: {photo.rejectReason || 'Guidelines violation'}
                                </p>
                              </div>
                            )}
                          </div>
                        ...
                        {event.winnersPublished && photo.scores?.length > 0 && (
                          <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-800 text-[10px] bg-slate-50 dark:bg-slate-900/50 p-2.5 rounded-lg">
                            <span className="font-bold text-indigo-600 dark:text-indigo-400">Judge Feedback:</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden p-6 sm:p-8 flex flex-col gap-6 animate-in zoom-in-95 duration-200">
            <div className="text-center flex flex-col gap-1 items-center">
              ...
              <h3 className="font-display font-extrabold text-lg text-slate-900 dark:text-white">Secure Checkout Gateway</h3>
              <p className="text-xs text-slate-400">Select simulated payment option to complete booking</p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl flex justify-between items-center border border-slate-100 dark:border-slate-850">
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wide">Selected Package</span>
                  <p className="text-sm font-semibold text-slate-900 dark:text-white">{selectedPackage?.name}</p>
                </div>
                <div className="text-right">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wide">Total Fee</span>
                  <p className="text-lg font-black text-indigo-600 dark:text-indigo-400">₹{selectedPackage?.price}.00</p>
                </div>
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-slate-500">Payment Option</label>
              ...
              {paymentMethod === 'Card' && (
                <div className="flex flex-col gap-3 p-3 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-850 rounded-2xl animate-in slide-in-from-bottom-2 duration-200">
                  <div className="flex flex-col gap-1 text-[11px]">
                    <label className="font-semibold text-slate-400">Card Number</label>
                    <input 
                      type="text" 
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    <div className="flex flex-col gap-1">
                      <label className="font-semibold text-slate-400">Expiry (MM/YY)</label>
                      <input 
                        type="text" 
                        placeholder="12/29" 
                        className="px-3 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg focus:outline-none"
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="font-semibold text-slate-400">CVV</label>
                      <input 
                        type="password" 
                        placeholder="•••" 
                        className="px-3 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg focus:outline-none"
              ...
              {paymentMethod === 'UPI' && (
                <div className="flex flex-col gap-2 p-3 bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-850 rounded-2xl text-[11px] animate-in slide-in-from-bottom-2 duration-200">
                  <label className="font-semibold text-slate-400">UPI Address ID</label>
                  <input 
                    type="text" 
